            ...
            <div class="content-header row">
                <div class="content-header-left col-md-9 col-12 mb-2">
                    <div class="row breadcrumbs-top">
                        <div class="col-12">
                            <h2 class="content-header-title float-start mb-0">PPPI</h2>
                            <div class="breadcrumb-wrapper">
                                <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item active">Pelaksanaan Pengelolaan Perubahan Iklim
                                    </li>
                                </ol>
                            </div>
                        </div>
                ...
                <div class="modal-dialog modal-dialog-centered modal-lg">
                    <div class="modal-content">
                        ...
                        <div class="modal-body pb-5 px-sm-4 mx-50">
                            <h3 class="address-title text-center mb-1" id="addNewAddressTitle">Pelaksanaan Pengelolaan
                                Perubahan Iklim</h3>
                            <p class="address-subtitle text-center mb-2 pb-75"></p>

                            <form action="/Lingkungan/p4a" method="POST" enctype="multipart/form-data"
                                ...
                                <div class="col-12 col-md-12">
                                    ...
                                    <select class="form-select" id="divisi" name="divisi">
                                        ...
                                        <option value="PT. PLN (Persero) UPDL BANJARBARU">PT. PLN (Persero) UPDL BANJARBARU
                                        </option>
                                        <option value="PT. PLN (Persero) UPDL BOGOR">PT. PLN (Persero) UPDL BOGOR</option>
                                        <option value="PT. PLN (Persero) UPDL JAKARTA">PT. PLN (Persero) UPDL JAKARTA
                                        </option>
                                        <option value="PT. PLN (Persero) UPDL MAKASAR">PT. PLN (Persero) UPDL MAKASAR
                                        </option>
                                        <option value="PT. PLN (Persero) UPDL PADANG">PT. PLN (Persero) UPDL PADANG</option>
                                        <option value="PT. PLN (Persero) UPDL PALEMBANG">PT. PLN (Persero) UPDL PALEMBANG
                                        </option>
                                        <option value="PT. PLN (Persero) UPDL PANDAAN">PT. PLN (Persero) UPDL PANDAAN
                                        </option>
                                        <option value="PT. PLN (Persero) UPDL SEMARANG">PT. PLN (Persero) UPDL SEMARANG
                                        </option>
                                        <option value="PT. PLN (Persero) UPDL SURALAYA">PT. PLN (Persero) UPDL SURALAYA
                                        </option>
                                        <option value="PT. PLN (Persero) UPDL TUNTUNGAN">PT. PLN (Persero) UPDL TUNTUNGAN
                                        </option>
                                    </select>
                                </div>
                                ...
                                <tbody>
                                    @foreach ($p4as as $p4a)
                                        <tr>
                                            ...
                                            <td class="text-center">{{ $p4a->nomer_surat }}</td>
                                            <td class="text-center">{{ $p4a->perihal }}</td>
                                            <td class="text-center">{{ $p4a->wulan }}</td>
                                            {{-- <td class="text-center">{{ $p4a->bulan }}</td> --}}
                                            <td class="text-center">{{ $p4a->tanggal_dibuat }}</td>
                                            <td class="text-center">
                                                @if ($p4a->status == 'diajukan')
                                                    <span class="badge rounded-pill  badge-light-info">Diajukan</span>
                                                @elseif ($p4a->status == 'disetujui')
                                                    <span class="badge rounded-pill  badge-light-success">Disetujui</span>
                                                @elseif ($p4a->status == 'direvisi')
                                                    <span class="badge rounded-pill  badge-light-warning">Revisi</span>
                                                @elseif ($p4a->status == 'ditindak')
                                                    <span class="badge rounded-pill  badge-light-primary">Tindak
                                                        Lanjut</span>
                                                @elseif ($p4a->status == 'ditolak')
                                                    <span class="badge rounded-pill  badge-light-danger">Ditolak</span>
                                                @endif
                                            </td>
                                            <td class="text-center">{{ $p4a->catatan }}</td>
                                            <td class="text-center">
                                                <a href="{{ asset('storage/' . $p4a->berkas) }}" target="blank"
                                                    class="btn btn-icon btn-success"><span data-feather="eye"></span></a>
                                                <a href="/Lingkungan/p4a/{{ $p4a->id }}/edit"
                                                    class="btn btn-icon btn-warning" style="margin: 3px 0px"><span
                                                        data-feather="edit"></span></a>

                                                <form action="/Lingkungan/p4a/{{ $p4a->id }}" method="post"
                                                    class="d-inline">
                                                    @method('delete')
                                                    @csrf

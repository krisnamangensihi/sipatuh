                        ...
                        <div class="col-12">
                            ...
                            <div class="breadcrumb-wrapper">
                                <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item active">Pemenuhan Izin Perlindungan dan Pengelolaan Lingkungan Hidup
                                    </li>
                                    <li class="breadcrumb-item active">Penyimpanan Limbah B3
                                    </li>
                                </ol>
                            </div>
                        </div>
                ...
                <div class="modal-dialog modal-dialog-centered modal-lg">
                    <div class="modal-content">
                        ...
                        <div class="modal-body pb-5 px-sm-4 mx-50">
                            <h3 class="address-title text-center mb-1" id="addNewAddressTitle">Penyimpanan Limbah B3</h3>
                            <p class="address-subtitle text-center mb-2 pb-75"></p>

                            <form action="/Lingkungan/p2d" method="POST" enctype="multipart/form-data"
                                ...
                                <tbody>
                                    @foreach ($p2ds as $p2d)
                                        <tr>
                                            ...
                                            <td class="text-center">{{ $p2d->nomer_surat }}</td>
                                            <td class="text-center">{{ $p2d->perihal }}</td>
                                            <td class="text-center">{{ $p2d->wulan }}</td>
                                            {{-- <td class="text-center">{{ $p2d->bulan }}</td> --}}
                                            <td class="text-center">{{ $p2d->tanggal_dibuat }}</td>
                                            <td class="text-center">
                                                @if ($p2d->status == 'diajukan')
                                                    <span class="badge rounded-pill  badge-light-info">Diajukan</span>
                                                @elseif ($p2d->status == 'disetujui')
                                                    <span class="badge rounded-pill  badge-light-success">Disetujui</span>
                                                @elseif ($p2d->status == 'direvisi')
                                                    <span class="badge rounded-pill  badge-light-warning">Revisi</span>
                                                @elseif ($p2d->status == 'ditindak')
                                                    <span class="badge rounded-pill  badge-light-primary">Tindak Lanjut</span>
                                                @elseif ($p2d->status == 'ditolak')
                                                    <span class="badge rounded-pill  badge-light-danger">Ditolak</span>
                                                @endif
                                            </td>
                                            <td class="text-center">{{ $p2d->catatan }}</td>
                                            <td class="text-center">
                                                <a href="{{ asset('storage/' . $p2d->berkas) }}" target="blank"
                                                    class="btn btn-icon btn-success"><span data-feather="eye"></span></a>
                                                <a href="/Lingkungan/p2d/{{ $p2d->id }}/edit"
                                                    class="btn btn-icon btn-warning" style="margin: 3px 0px"><span
                                                        data-feather="edit"></span></a>

                                                <form action="/Lingkungan/p2d/{{ $p2d->id }}" method="post"
                                                    class="d-inline">
                                                    @method('delete')
                                                    @csrf

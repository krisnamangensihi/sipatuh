                    ...
                    <div class="modal-dialog modal-dialog-centered modal-lg">
                        <div class="modal-content">
                            ...
                            <div class="modal-body pb-5 px-sm-4 mx-50">
                                <h3 class="address-title text-center mb-1" id="addNewAddressTitle">Neraca Limbah B3
                                </h3>
                                <div class="mb-1">
                                    <p><a href="https://docs.google.com/spreadsheets/d/17XhrOMURzm_DkvaM6_HjZGvZGtbEw6Q1/edit?usp=sharing&ouid=104644451334503581170&rtpof=true&sd=true"
                                            target="blank"><strong>Unduh Format Neraca LB3 Disini</strong></a>
                                    </p>
                                </div>
                                {{-- <p class="address-subtitle text-center mb-2 pb-75"></p> --}}

                                <form action="/Lingkungan/p5b" method="POST" enctype="multipart/form-data" id="addNewAddressForm"
                                    class="row gy-1 gx-2">
                                    @csrf
                                    <div class="col-12">
                                ...
                                <tbody>
                                    @foreach ($p5bs as $p5b)
                                        <tr>
                                            ...
                                            <td class="text-center">{{ $p5b->perihal }}</td>
                                            {{-- <td class="text-center">{{ $p5b->dok }}</td> --}}
                                            {{-- <td class="text-center">{{ $p5b->divisi }}</td> --}}
                                            <td class="text-center">{{ $p5b->wulan }}</td>
                                            {{-- <td class="text-center">{{ $p5b->bulan }}</td> --}}
                                            <td class="text-center">{{ $p5b->tanggal_dibuat }}</td>
                                            <td class="text-center">
                                                @if ($p5b->status == 'diajukan')
                                                    <span class="badge rounded-pill  badge-light-info">Diajukan</span>
                                                @elseif ($p5b->status == 'disetujui')
                                                    <span class="badge rounded-pill  badge-light-success">Disetujui</span>
                                                @elseif ($p5b->status == 'direvisi')
                                                    <span class="badge rounded-pill  badge-light-warning">Revisi</span>
                                                @elseif ($p5b->status == 'ditindak')
                                                    <span class="badge rounded-pill  badge-light-primary">Tindak Lanjut</span>
                                                @elseif ($p5b->status == 'ditolak')
                                                    <span class="badge rounded-pill  badge-light-danger">Ditolak</span>
                                                @endif
                                            </td>
                                            <td class="text-center">{{ $p5b->catatan }}</td>
                                            <td class="text-center">
                                                <a href="{{ asset('storage/' . $p5b->berkas) }}" target="blank"
                                                    class="btn btn-icon btn-success"><span data-feather="eye"></span></a>
                                                <a href="/Lingkungan/p5b/{{ $p5b->id }}/edit"
                                                    class="btn btn-icon btn-warning" style="margin: 3px 0px"><span
                                                        data-feather="edit"></span></a>

                                                <form action="/Lingkungan/p5b/{{ $p5b->id }}" method="post"
                                                    class="d-inline">
                                                    @method('delete')
                                                    @csrf

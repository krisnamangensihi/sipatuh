...
<body>
    ...
    <div id=border>
        ...
        <div class="barcode">
            <p></p>
            @foreach ($kepemimpinan as $kepim)
                @if ($kepim->verifikasi == 'tw1')
                    <img src="data:image/png;base64,{!! base64_encode(
                        QrCode::format('svg')->size(100)->generate("disetujui oleh:,$kepim->name,NIP:,$kepim->nip"),
                    ) !!}">
                    <p class="text-center">{{ $kepim->name }}</p>
                @elseif ($kepim->verifikasi == 'tw2')
                    <img src="data:image/png;base64,{!! base64_encode(
                        QrCode::format('svg')->size(100)->generate("disetujui oleh:,$kepim->name,NIP:,$kepim->nip"),
                    ) !!}">
                    <p>{{ $kepim->name }}</p>
                @elseif ($kepim->verifikasi == 'tw3')
                    <img src="data:image/png;base64,{!! base64_encode(
                        QrCode::format('svg')->size(100)->generate("disetujui oleh:,$kepim->name,NIP:,$kepim->nip"),
                    ) !!}">
                @elseif ($kepim->verifikasi == 'tw4')
                    <img src="data:image/png;base64,{!! base64_encode(
                        QrCode::format('svg')->size(100)->generate("disetujui oleh:,$kepim->name,NIP:,$kepim->nip"),
                    ) !!}">
                @endif
